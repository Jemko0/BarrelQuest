---@meta

--- NOTE: In this file, underscores (_) in names represent spaces from Blueprint.
--- For example: 'Apply_Damage' in Lua corresponds to 'Apply Damage' in Blueprint.
--- Access these using bracket notation: object["Apply Damage"]

---@diagnostic disable: undefined-doc-name

---@class SkeletalMeshComponent : SkinnedMeshComponent
---SkeletalMeshComponent is used to create an instance of an animated SkeletalMesh asset.
---@see https://docs.unrealengine.com/latest/INT/Engine/Content/Types/SkeletalMeshes/
---@see USkeletalMesh
---
--- Properties
---
---The blueprint for creating an AnimationScript.
---@field AnimationBlueprint AnimBlueprint
---@field AnimBlueprintGeneratedClass AnimBlueprintGeneratedClass
---The AnimBlueprint class to use. Use 'SetAnimInstanceClass' to change at runtime.
---@field AnimClass Class
---The active animation graph program instance.
---@field AnimScriptInstance AnimInstance
---@field SubInstances AnimInstance[]
---Post-processing AnimBP to use for the given skeletal mesh component, overriding the one set in the skeletal mesh asset.
---@field OverridePostProcessAnimBP Class
---An instance created from the PostPhysicsBlueprint property of the skeletal mesh we're using,
---Runs after (and receives pose from) the main anim instance.
---@field PostProcessAnimInstance AnimInstance
---@field AnimationData SingleAnimationPlayData
---Offset of the root bone from the reference pose. Used to offset bounding box.
---@field RootBoneTranslation Vector
---If bEnableLineCheckWithBounds is true, scale the bounds by this value before doing line check.
---@field LineCheckBoundsScale Vector
---Used to scale speed of all animations on this skeletal mesh.
---@field GlobalAnimRateScale number
---If we are running physics, should we update non-simulated bones based on the animation bone positions.
---@field KinematicBonesUpdateType integer
---Whether physics simulation updates component transform.
---@field PhysicsTransformUpdateMode integer
---whether we need to teleport cloth. // This property is explicitly hidden from the details panel inside FSkeletalMeshComponentDetails::UpdatePhysicsCategory
---@field ClothTeleportMode EClothingTeleportMode
---Whether to use Animation Blueprint or play Single Animation Asset.
---@field AnimationMode integer
---Controls whether blending in physics bones will refresh overlaps on this component, defaults to true but can be disabled in cases where we know anim->physics blending doesn't meaningfully change overlaps
---@field bUpdateOverlapsOnAnimationFinalize boolean
---If true, there is at least one body in the current PhysicsAsset with a valid bone in the current SkeletalMesh
---@field bHasValidBodies boolean
---Forces use of the physics bodies irrespective of whether they are simulated or using the physics blend weight
---@field bBlendPhysics boolean
---If true, simulate physics for this component on a dedicated server.
---This should be set if simulating physics and replicating with a dedicated server.
---    Note: This property cannot be changed at runtime.
---@field bEnablePhysicsOnDedicatedServer boolean
---If true, then the physics bodies will be used to drive the skeletal mesh even when they are
---kinematic (not simulating), otherwise the skeletal mesh will be driven by the animation input
---when the bodies are kinematic
---@field bUpdateMeshWhenKinematic boolean
---If we should pass joint position to joints each frame, so that they can be used by motorized joints to drive the
---ragdoll based on the animation.
---@field bUpdateJointsFromAnimation boolean
---Toggles creation of cloth simulation. Distinct from the simulation toggle below in that, if off, avoids allocating
---the actors entirely instead of just skipping the simulation step.
---@field bAllowClothActors boolean
---Disable cloth simulation and play original animation without simulation
---@field bDisableClothSimulation boolean
---can't collide with part of environment if total collision volumes exceed 16 capsules or 32 planes per convex
---@field bCollideWithEnvironment boolean
---can't collide with part of attached children if total collision volumes exceed 16 capsules or 32 planes per convex
---@field bCollideWithAttachedChildren boolean
---Forces the cloth simulation to constantly update its external collisions, at the expense of performance.
---This will help to prevent missed collisions if the cloth's skeletal mesh component isn't moving,
---and when instead, wind or other moving objects are causing new collision shapes to come into the cloth's vicinity.
---@field bForceCollisionUpdate boolean
---Scale applied to the component induced velocity of all cloth particles prior to stepping the cloth solver.
---Use 1.0 for fully induced velocity (default), or use 0.0 for no induced velocity, and any other values in between for a reduced induced velocity.
---When set to 0.0, it also provides a way to force the clothing to simulate in local space.
---This value multiplies to individual cloth's velocity scale settings, still allowing for differences in behavior between the various pieces of clothing within the same component.
---@field ClothVelocityScale number
---reset the clothing after moving the clothing position (called teleport)
---@field bResetAfterTeleport boolean
---Whether animation and world transform updates are deferred. If this is on, the kinematic bodies (scene query data) will not update until the next time the physics simulation is run
---@field bDeferKinematicBoneUpdate boolean
---Skips Ticking and Bone Refresh.
---@field bNoSkeletonUpdate boolean
---pauses this component's animations (doesn't tick them, but still refreshes bones)
---@field bPauseAnims boolean
---Whether the built-in animation of this component should run when the component ticks.
---It is assumed that if this is false then some external system will be animating this mesh.
---Note that disabling animation will also cause cloth simulation not to run and the component's tick to run on any thread.
---@field bEnableAnimation boolean
---On InitAnim should we set to ref pose (if false use first tick of animation data). If enabled, takes precedence over UAnimationSettings::bTickAnimationOnSkeletalMeshInit
---@field bUseRefPoseOnInitAnim boolean
---Uses skinned data for collision data.
---@field bEnablePerPolyCollision boolean
---If true, force the mesh into the reference pose - is an optimization.
---@field bForceRefpose boolean
---If true TickPose() will not be called from the Component's TickComponent function.
---It will instead be called from Autonomous networking updates. See ACharacter.
---@field bOnlyAllowAutonomousTickPose boolean
---True if calling TickPose() from Autonomous networking updates. See ACharacter.
---@field bIsAutonomousTickPose boolean
---If bForceRefPose was set last tick.
---@field bOldForceRefPose boolean
---Bool that enables debug drawing of the skeleton before it is passed to the physics. Useful for debugging animation-driven physics.
---@field bShowPrePhysBones boolean
---If false, indicates that on the next call to UpdateSkelPose the RequiredBones array should be recalculated.
---@field bRequiredBonesUpToDate boolean
---If true, AnimTree has been initialised.
---@field bAnimTreeInitialised boolean
---If true, line checks will test against the bounding box of this skeletal mesh component and return a hit if there is a collision.
---@field bEnableLineCheckWithBounds boolean
---If true, propagates calls to ApplyAnimationCurvesToComponent for follower components, only needed if follower components do not tick themselves
---@field bPropagateCurvesToFollowers boolean
---Whether to skip UpdateKinematicBonesToAnim() when interpolating. Kinematic bones are updated to the target interpolation pose only on ticks when they are evaluated.
---@field bSkipKinematicUpdateWhenInterpolating boolean
---Whether to skip bounds update when interpolating. Bounds are updated to the target interpolation pose only on ticks when they are evaluated.
---@field bSkipBoundsUpdateWhenInterpolating boolean
---If true, this will Tick until disabled
---@field bUpdateAnimationInEditor boolean
---If true, will play cloth in editor
---@field bUpdateClothInEditor boolean
---If true, DefaultAnimatingRigOverride will be used. If false, use the DefaultAnimatingRig in the SkeletalMesh
---@field bOverrideDefaultAnimatingRig boolean
---Cache AnimCurveUidVersion from Skeleton and this will be used to identify if it needs to be updated
---@field CachedAnimCurveUidVersion any
---weight to blend between simulated results and key-framed positions
---if weight is 1.0, shows only cloth simulation results and 0.0 will show only skinned results
---@field ClothBlendWeight number
---Whether we should stall the Cloth tick task until the cloth simulation is complete. This is required if we want up-to-date
---cloth data on the game thread, for example if we want to generate particles at cloth vertices.
---@field bWaitForParallelClothTask boolean
---Used for per poly collision. In 99% of cases you will be better off using a Physics Asset.
---This BodySetup is per instance because all modification of vertices is done in place
---@field BodySetup BodySetup
---@field ClothMaxDistanceScale number
---This scale is applied to all cloth geometry (e.g., cloth meshes and collisions) in order to simulate in a different scale space than world.This scale is not applied to distance-based simulation parameters such as MaxDistance.
---This property is currently only read by the cloth solver when creating cloth actors, but may become animatable in the future.
---@field ClothGeometryScale number
---Notification when constraint is broken.
---@field OnConstraintBroken function
---Notification when constraint plasticity drive target changes.
---@field OnPlasticDeformation function
---Class of the object responsible for
---@field ClothingSimulationFactory Class
---Broadcast when the components anim instance is initialized
---@field OnAnimInitialized function
local SkeletalMeshComponent = {}

--- Methods
---Runs through all layer nodes, attempting to find layer nodes that are currently running the specified class, then resets each to its default value.
---State sharing rules are as with SetLayerOverlay.
---If InClass is null, does nothing.
---@param InClass Class
---@return nil
function SkeletalMeshComponent.UnlinkAnimClassLayers(InClass) end

---If this component has a valid LeaderPoseComponent and has previously had its cloth bound to the
---MCP, this function will unbind the cloth and resume simulation.
---space, we will restore this setting. This will cause the leader component to reset which may be
---undesirable.
---@param bRestoreSimulationSpace boolean
---@return nil
function SkeletalMeshComponent.UnbindClothFromLeaderPoseComponent(bRestoreSimulationSpace) end

---Toggles whether the post process blueprint will run for this component
---@return nil
function SkeletalMeshComponent.ToggleDisablePostProcessBlueprint() end

---Terminate physics on all bodies below the named bone, effectively disabling collision forever. If you terminate, you won't be able to re-init later.
---@param ParentBoneName string
---@return nil
function SkeletalMeshComponent.TermBodiesBelow(ParentBoneName) end

---Stops simulating clothing, but does not show clothing ref pose. Keeps the last known simulation state
---@return nil
function SkeletalMeshComponent.SuspendClothingSimulation() end

---Animation play functions
---      *
---      * These changes status of animation instance, which is transient data, which means it won't serialize with this component
---      * Because of that reason, it is not safe to be used during construction script
---      * Please use OverrideAnimationData for construction script. That will override AnimationData to be serialized
---@return nil
function SkeletalMeshComponent.Stop() end

---Takes a snapshot of this skeletal mesh component's pose and saves it to the specified snapshot.
---The snapshot is taken at the current LOD, so if for example you took the snapshot at LOD1
---and then used it at LOD0 any bones not in LOD1 will use the reference pose
---@return nil, PoseSnapshot
function SkeletalMeshComponent.SnapshotPose() end

---Sets whether or not to animate cloth in the editor. Requires Update Animation In Editor to also be true.
---This is supported only in the editor
---@param NewUpdateState boolean
---@return nil
function SkeletalMeshComponent.SetUpdateClothInEditor(NewUpdateState) end

---Sets whether or not to force tick component in order to update animation and refresh transform for this component
---This is supported only in the editor
---@param NewUpdateState boolean
---@return nil
function SkeletalMeshComponent.SetUpdateAnimationInEditor(NewUpdateState) end

---Sets the teleportation rotation threshold.
---@param Threshold number
---@return nil
function SkeletalMeshComponent.SetTeleportRotationThreshold(Threshold) end

---Sets the teleportation distance threshold.
---@param Threshold number
---@return nil
function SkeletalMeshComponent.SetTeleportDistanceThreshold(Threshold) end

---Set the SkeletalMesh rendered for this mesh.
---@param NewMesh SkeletalMesh
---@return nil
function SkeletalMeshComponent.SetSkeletalMeshAsset(NewMesh) end

---Animation play functions
---      *
---      * These changes status of animation instance, which is transient data, which means it won't serialize with this component
---      * Because of that reason, it is not safe to be used during construction script
---      * Please use OverrideAnimationData for construction script. That will override AnimationData to be serialized
---@param InPos number
---@param bFireNotifies boolean
---@return nil
function SkeletalMeshComponent.SetPosition(InPos, bFireNotifies) end

---Animation play functions
---      *
---      * These changes status of animation instance, which is transient data, which means it won't serialize with this component
---      * Because of that reason, it is not safe to be used during construction script
---      * Please use OverrideAnimationData for construction script. That will override AnimationData to be serialized
---@param Rate number
---@return nil
function SkeletalMeshComponent.SetPlayRate(Rate) end

---This is global set up for setting physics blend weight
---This does multiple things automatically
---If PhysicsBlendWeight == 1.f, it will enable Simulation, and if PhysicsBlendWeight == 0.f, it will disable Simulation.
---Also it will respect each body's setup, so if the body is fixed, it won't simulate. Vice versa
---So if you'd like all bodies to change manually, do not use this function, but SetAllBodiesPhysicsBlendWeight
---@param PhysicsBlendWeight number
---@return nil
function SkeletalMeshComponent.SetPhysicsBlendWeight(PhysicsBlendWeight) end

---Set the post-processing AnimBP to be used for this skeletal mesh component.
---In case an override post-processing AnimBP is set, the one set in skeletal mesh asset will be ignored and not used.
---                                                       the game is running and the anim instances need to be re-initialized.
---@param InPostProcessAnimBlueprint Class
---@param ReinitAnimInstances boolean
---@return nil
function SkeletalMeshComponent.SetOverridePostProcessAnimBP(InPostProcessAnimBlueprint, ReinitAnimInstances) end

---Changes the value of bNotifyRigidBodyCollision on all bodies below a given bone
---@param bNewNotifyRigidBodyCollision boolean
---@param BoneName string
---@param bIncludeSelf boolean
---@return nil
function SkeletalMeshComponent.SetNotifyRigidBodyCollisionBelow(bNewNotifyRigidBodyCollision, BoneName, bIncludeSelf) end

---Set Morph Target with Name and Value(0-1)
---@param MorphTargetName string
---@param Value number
---@param bRemoveZeroWeight boolean
---@return nil
function SkeletalMeshComponent.SetMorphTarget(MorphTargetName, Value, bRemoveZeroWeight) end

---Disable physics blending of bones *
---@param bNewBlendPhysics boolean
---@return nil
function SkeletalMeshComponent.SetEnablePhysicsBlending(bNewBlendPhysics) end

---Enables or disables gravity to all bodies below the given bone.
---NAME_None indicates all bodies will be edited.
---In that case, consider using UPrimitiveComponent::EnableGravity.
---@param bEnableGravity boolean
---@param BoneName string
---@param bIncludeSelf boolean
---@return nil
function SkeletalMeshComponent.SetEnableGravityOnAllBodiesBelow(bEnableGravity, BoneName, bIncludeSelf) end

---Enables or disables gravity for the given bone.
---NAME_None indicates the root body will be edited.
---If the bone name given is otherwise invalid, nothing happens.
---@param bEnableGravity boolean
---@param BoneName string
---@return nil
function SkeletalMeshComponent.SetEnableBodyGravity(bEnableGravity, BoneName) end

---Whether the built-in animation of this component should run when the component ticks. See USkeletalMeshComponent::bEnableAnimation for more details.
---Note that this call defers setting the flag to the end of the frame as the flag cannot change during world ticking (as other tasks may be in flight)
---@param bNewEnableAnimation boolean
---@return nil
function SkeletalMeshComponent.SetEnableAnimation(bNewEnableAnimation) end

---Sets whether the post process blueprint is currently running for this component.
---If it is not currently running, and is set to run, the instance will be reinitialized
---@param bInDisablePostProcess boolean
---@return nil
function SkeletalMeshComponent.SetDisablePostProcessBlueprint(bInDisablePostProcess) end

---Set Disable Anim Curves
---@param bInDisableAnimCurves boolean
---@return nil
function SkeletalMeshComponent.SetDisableAnimCurves(bInDisableAnimCurves) end

---Set Default Animating Rig Override
---@param InAnimatingRig any
---@return nil
function SkeletalMeshComponent.SetDefaultAnimatingRigOverride(InAnimatingRig) end

---Sets the constraint profile properties (limits, motors, etc...) to match the constraint profile as defined in the physics asset for all constraints. If profile name is not found the joint is set to use the default constraint profile.
---@param ProfileName string
---@param bDefaultIfNotFound boolean
---@return nil
function SkeletalMeshComponent.SetConstraintProfileForAll(ProfileName, bDefaultIfNotFound) end

---Sets the constraint profile properties (limits, motors, etc...) to match the constraint profile as defined in the physics asset. If profile name is not found the joint is set to use the default constraint profile.
---@param JointName string
---@param ProfileName string
---@param bDefaultIfNotFound boolean
---@return nil
function SkeletalMeshComponent.SetConstraintProfile(JointName, ProfileName, bDefaultIfNotFound) end

---Set Cloth Max Distance Scale
---@param Scale number
---@return nil
function SkeletalMeshComponent.SetClothMaxDistanceScale(Scale) end

---Set a single bone to be simulated (or not)
---@param bSimulate boolean
---@return nil
function SkeletalMeshComponent.SetBodySimulatePhysics(bSimulate) end

---Changes the value of bNotifyRigidBodyCollision for a given body
---@param bNewNotifyRigidBodyCollision boolean
---@param BoneName string
---@return nil
function SkeletalMeshComponent.SetBodyNotifyRigidBodyCollision(bNewNotifyRigidBodyCollision, BoneName) end

---Set the anim instance class. Clears and re-initializes the anim instance with the new class and sets animation mode to 'AnimationBlueprint'
---@param NewClass Class
---@return nil
function SkeletalMeshComponent.SetAnimInstanceClass(NewClass) end

---Set the Animation Mode
---@param InAnimationMode integer
---@param bForceInitAnimScriptInstance boolean
---@return nil
function SkeletalMeshComponent.SetAnimationMode(InAnimationMode, bForceInitAnimScriptInstance) end

---Animation play functions
---      *
---      * These changes status of animation instance, which is transient data, which means it won't serialize with this component
---      * Because of that reason, it is not safe to be used during construction script
---      * Please use OverrideAnimationData for construction script. That will override AnimationData to be serialized
---@param NewAnimToPlay AnimationAsset
---@return nil
function SkeletalMeshComponent.SetAnimation(NewAnimToPlay) end

---Sets the Angular Motion Ranges for a named constraint
---@param InBoneName string
---@param Swing1LimitAngle number
---@param TwistLimitAngle number
---@param Swing2LimitAngle number
---@return nil
function SkeletalMeshComponent.SetAngularLimits(InBoneName, Swing1LimitAngle, TwistLimitAngle, Swing2LimitAngle) end

---Sets whether or not to allow rigid body animation nodes for this component
---@param bInAllow boolean
---@param bReinitAnim boolean
---@return nil
function SkeletalMeshComponent.SetAllowRigidBodyAnimNode(bInAllow, bReinitAnim) end

---resets, and then only allow the following list to be allowed/disallowed
---@param bAllow boolean
---@return nil
function SkeletalMeshComponent.SetAllowedAnimCurvesEvaluation(bAllow) end

---Sets whether cloth assets should be created/simulated in this component.
---This will update the conditional flag and you will want to call RecreateClothingActors for it to take effect.
---@param bInAllow boolean
---@return nil
function SkeletalMeshComponent.SetAllowClothActors(bInAllow) end

---Set Allow Anim Curve Evaluation
---@param bInAllow boolean
---@return nil
function SkeletalMeshComponent.SetAllowAnimCurveEvaluation(bInAllow) end

---Enable or Disable AngularVelocityDrive. If motor is in SLERP mode it will be turned on if either EnableSwingDrive OR EnableTwistDrive are enabled. In Twist and Swing mode the twist and the swing can be controlled individually.
---@param bEnableSwingDrive boolean
---@param bEnableTwistDrive boolean
---@param bSkipCustomPhysicsType boolean
---@return nil
function SkeletalMeshComponent.SetAllMotorsAngularVelocityDrive(bEnableSwingDrive, bEnableTwistDrive, bSkipCustomPhysicsType) end

---Enable or Disable AngularPositionDrive. If motor is in SLERP mode it will be turned on if either EnableSwingDrive OR EnableTwistDrive are enabled. In Twist and Swing mode the twist and the swing can be controlled individually.
---@param bEnableSwingDrive boolean
---@param bEnableTwistDrive boolean
---@param bSkipCustomPhysicsType boolean
---@return nil
function SkeletalMeshComponent.SetAllMotorsAngularPositionDrive(bEnableSwingDrive, bEnableTwistDrive, bSkipCustomPhysicsType) end

---Set Angular Drive motors params for all constraint instances
---@param InSpring number
---@param InDamping number
---@param InForceLimit number
---@param bSkipCustomPhysicsType boolean
---@return nil
function SkeletalMeshComponent.SetAllMotorsAngularDriveParams(InSpring, InDamping, InForceLimit, bSkipCustomPhysicsType) end

---Set bSimulatePhysics to true for all bone bodies. Does not change the component bSimulatePhysics flag.
---@param bNewSimulate boolean
---@return nil
function SkeletalMeshComponent.SetAllBodiesSimulatePhysics(bNewSimulate) end

---Set All Bodies Physics Blend Weight
---@param PhysicsBlendWeight number
---@param bSkipCustomPhysicsType boolean
---@return nil
function SkeletalMeshComponent.SetAllBodiesPhysicsBlendWeight(PhysicsBlendWeight, bSkipCustomPhysicsType) end

---Set all of the bones below passed in bone to be simulated
---@param bNewSimulate boolean
---@param bIncludeSelf boolean
---@return nil
function SkeletalMeshComponent.SetAllBodiesBelowSimulatePhysics(bNewSimulate, bIncludeSelf) end

---[WARNING: Chaos Only]
---Set all of the bones below passed in bone to be disabled or not for the associated physics solver
---Bodies will not be colliding or be part of the physics simulation.
---This is different from SetAllBodiesBelowSimulatePhysics that changes bodies to Kinematic/simulated
---@param bDisabled boolean
---@param bIncludeSelf boolean
---@return nil
function SkeletalMeshComponent.SetAllBodiesBelowPhysicsDisabled(bDisabled, bIncludeSelf) end

---Set all of the bones below passed in bone to be simulated
---@param PhysicsBlendWeight number
---@param bSkipCustomPhysicsType boolean
---@param bIncludeSelf boolean
---@return nil
function SkeletalMeshComponent.SetAllBodiesBelowPhysicsBlendWeight(PhysicsBlendWeight, bSkipCustomPhysicsType, bIncludeSelf) end

---set the linear velocity of the child bodies to match that of the given parent bone
---@param bIncludeSelf boolean
---@return nil
function SkeletalMeshComponent.SetAllBodiesBelowLinearVelocity(bIncludeSelf) end

---Resumes a previously suspended clothing simulation, teleporting the clothing on the next tick
---@return nil
function SkeletalMeshComponent.ResumeClothingSimulation() end

---Reset the teleport mode of a next update to 'Continuous'
---@return nil
function SkeletalMeshComponent.ResetClothTeleportMode() end

---Remove all cloth collision sources
---@return nil
function SkeletalMeshComponent.ResetClothCollisionSources() end

---Informs any active anim instances (main instance, linked instances, post instance) that a dynamics reset is required
---for example if a teleport occurs.
---@param InTeleportType ETeleportType
---@return nil
function SkeletalMeshComponent.ResetAnimInstanceDynamics(InTeleportType) end

---By reset, it will allow all the curves to be evaluated
---@return nil
function SkeletalMeshComponent.ResetAllowedAnimCurveEvaluation() end

---Allows you to reset bodies Simulate state based on where bUsePhysics is set to true in the BodySetup.
---@return nil
function SkeletalMeshComponent.ResetAllBodiesSimulatePhysics() end

---Remove a cloth collision source defined by a component
---@param InSourceComponent SkeletalMeshComponent
---@return nil
function SkeletalMeshComponent.RemoveClothCollisionSources(InSourceComponent) end

---Remove a cloth collision source defined by both a component and a physics asset
---@param InSourceComponent SkeletalMeshComponent
---@param InSourcePhysicsAsset PhysicsAsset
---@return nil
function SkeletalMeshComponent.RemoveClothCollisionSource(InSourceComponent, InSourcePhysicsAsset) end

---Destroys and recreates the clothing actors in the current simulation
---@return nil
function SkeletalMeshComponent.RecreateClothingActors() end

---Animation play functions
---       *
---       * These changes status of animation instance, which is transient data, which means it won't serialize with this component
---       * Because of that reason, it is not safe to be used during construction script
---       * Please use OverrideAnimationData for construction script. That will override AnimationData to be serialized
---@param NewAnimToPlay AnimationAsset
---@param bLooping boolean
---@return nil
function SkeletalMeshComponent.PlayAnimation(NewAnimToPlay, bLooping) end

---Animation play functions
---      *
---      * These changes status of animation instance, which is transient data, which means it won't serialize with this component
---      * Because of that reason, it is not safe to be used during construction script
---      * Please use OverrideAnimationData for construction script. That will override AnimationData to be serialized
---@param bLooping boolean
---@return nil
function SkeletalMeshComponent.Play(bLooping) end

---This overrides current AnimationData parameter in the SkeletalMeshComponent. This will serialize when the component serialize
---so it can be used during construction script. However note that this will override current existing data
---This can be useful if you'd like to make a blueprint with custom default animation per component
---This sets single player mode, which means you can't use AnimBlueprint with it
---@param InAnimToPlay AnimationAsset
---@param bIsLooping boolean
---@param bIsPlaying boolean
---@param Position number
---@param PlayRate number
---@return nil
function SkeletalMeshComponent.OverrideAnimationData(InAnimToPlay, bIsLooping, bIsPlaying, Position, PlayRate) end

---Runs through all nodes, attempting to find linked instance by name/tag, then sets the class of each node if the tag matches
---@param InTag string
---@param InClass Class
---@return nil
function SkeletalMeshComponent.LinkAnimGraphByTag(InTag, InClass) end

---Runs through all layer nodes, attempting to find layer nodes that are implemented by the specified class, then sets up a linked instance of the class for each.
---Allocates one linked instance to run each of the groups specified in the class, so state is shared. If a layer is not grouped (ie. NAME_None), then state is not shared
---and a separate linked instance is allocated for each layer node.
---If InClass is null, then all layers are reset to their defaults.
---@param InClass Class
---@return nil
function SkeletalMeshComponent.LinkAnimClassLayers(InClass) end

---Given a world position, find the closest point on the physics asset. Note that this is independent of collision and welding. This is based purely on animation position
---@return boolean
function SkeletalMeshComponent.K2_GetClosestPointOnPhysicsAsset() end

---Animation play functions
---      *
---      * These changes status of animation instance, which is transient data, which means it won't serialize with this component
---      * Because of that reason, it is not safe to be used during construction script
---      * Please use OverrideAnimationData for construction script. That will override AnimationData to be serialized
---@return boolean
function SkeletalMeshComponent.IsPlaying() end

---Gets whether or not the clothing simulation is currently suspended
---@return boolean
function SkeletalMeshComponent.IsClothingSimulationSuspended() end

---Checks whether or not gravity is enabled on the given bone.
---NAME_None indicates the root body should be queried.
---If the bone name given is otherwise invalid, false is returned.
---@param BoneName string
---@return boolean
function SkeletalMeshComponent.IsBodyGravityEnabled(BoneName) end

---Returns whether there are any valid instances to run, currently this means whether we have
---have an animation instance or a post process instance available to process.
---@return boolean
function SkeletalMeshComponent.HasValidAnimationInstance() end

---Get FTransform type attribute value.
---@param LookupType ECustomBoneAttributeLookup
---@return boolean
function SkeletalMeshComponent.GetTransformAttribute_Ref(LookupType) end

---Get FTransform type attribute value.
---@param DefaultValue Transform
---@param LookupType ECustomBoneAttributeLookup
---@return boolean
function SkeletalMeshComponent.GetTransformAttribute(DefaultValue, LookupType) end

---Gets the teleportation rotation threshold.
---@return number
function SkeletalMeshComponent.GetTeleportRotationThreshold() end

---Gets the teleportation distance threshold.
---@return number
function SkeletalMeshComponent.GetTeleportDistanceThreshold() end

---Get string type attribute value.
---@param LookupType ECustomBoneAttributeLookup
---@return boolean
function SkeletalMeshComponent.GetStringAttribute_Ref(LookupType) end

---Get string type attribute value.
---@param DefaultValue string
---@param LookupType ECustomBoneAttributeLookup
---@return boolean
function SkeletalMeshComponent.GetStringAttribute(DefaultValue, LookupType) end

---Get the SkeletalMesh rendered for this mesh.
---@return SkeletalMesh
function SkeletalMeshComponent.GetSkeletalMeshAsset() end

---Returns the center of mass of the skeletal mesh, instead of the root body's location
---@return Vector
function SkeletalMeshComponent.GetSkeletalCenterOfMass() end

---Returns the active post process instance is one is available. This is set on the mesh that this
---component is using, and is evaluated immediately after the main instance.
---@return AnimInstance
function SkeletalMeshComponent.GetPostProcessInstance() end

---Animation play functions
---      *
---      * These changes status of animation instance, which is transient data, which means it won't serialize with this component
---      * Because of that reason, it is not safe to be used during construction script
---      * Please use OverrideAnimationData for construction script. That will override AnimationData to be serialized
---@return number
function SkeletalMeshComponent.GetPosition() end

---Animation play functions
---      *
---      * These changes status of animation instance, which is transient data, which means it won't serialize with this component
---      * Because of that reason, it is not safe to be used during construction script
---      * Please use OverrideAnimationData for construction script. That will override AnimationData to be serialized
---@return number
function SkeletalMeshComponent.GetPlayRate() end

---Get Morph target with given name
---@param MorphTargetName string
---@return number
function SkeletalMeshComponent.GetMorphTarget(MorphTargetName) end

---Gets the layer linked instance corresponding to the specified group
---@param InGroup string
---@return AnimInstance
function SkeletalMeshComponent.GetLinkedAnimLayerInstanceByGroup(InGroup) end

---Gets the first layer linked instance corresponding to the specified class
---@param InClass Class
---@return AnimInstance
function SkeletalMeshComponent.GetLinkedAnimLayerInstanceByClass(InClass) end

---Get Linked Anim Graph Instances by Tag
---@param InTag string
---@return nil, AnimInstance[]
function SkeletalMeshComponent.GetLinkedAnimGraphInstancesByTag(InTag) end

---Returns the a tagged linked instance node. If no linked instances are found or none are tagged with the
---supplied name, this will return NULL.
---@param InTag string
---@return AnimInstance
function SkeletalMeshComponent.GetLinkedAnimGraphInstanceByTag(InTag) end

---Get integer type attribute value.
---@param LookupType ECustomBoneAttributeLookup
---@return boolean
function SkeletalMeshComponent.GetIntegerAttribute_Ref(LookupType) end

---Get integer type attribute value.
---@param DefaultValue integer
---@param LookupType ECustomBoneAttributeLookup
---@return boolean
function SkeletalMeshComponent.GetIntegerAttribute(DefaultValue, LookupType) end

---Get float type attribute value.
---@param LookupType ECustomBoneAttributeLookup
---@return boolean
function SkeletalMeshComponent.GetFloatAttribute_Ref(LookupType) end

---Get float type attribute value.
---@param DefaultValue number
---@param LookupType ECustomBoneAttributeLookup
---@return boolean
function SkeletalMeshComponent.GetFloatAttribute(DefaultValue, LookupType) end

---Gets whether the post process blueprint is currently disabled for this component
---@return boolean
function SkeletalMeshComponent.GetDisablePostProcessBlueprint() end

---Get Disable Anim Curves
---@return boolean
function SkeletalMeshComponent.GetDisableAnimCurves() end

---Get Default Animating Rig Override
---@return any
function SkeletalMeshComponent.GetDefaultAnimatingRigOverride() end

---Get Default Animating Rig
---@return any
function SkeletalMeshComponent.GetDefaultAnimatingRig() end

---Get curve value
---@param CurveName string
---@param DefaultValue number
---@return boolean
function SkeletalMeshComponent.GetCurveValue(CurveName, DefaultValue) end

---Gets the current Angular state for a named bone constraint
---@param InBoneName string
---@return nil, number, number, number
function SkeletalMeshComponent.GetCurrentJointAngles(InBoneName) end

---Gets all the constraints attached to a body
---@param BodyName string
---@param bParentConstraints boolean
---@param bChildConstraints boolean
---@param bIncludesTerminated boolean
---@return nil, ConstraintInstanceAccessor[]
function SkeletalMeshComponent.GetConstraintsFromBody(BodyName, bParentConstraints, bChildConstraints, bIncludesTerminated) end

---Gets all the constraints
---@param bIncludesTerminated boolean
---@return nil, ConstraintInstanceAccessor[]
function SkeletalMeshComponent.GetConstraints(bIncludesTerminated) end

---Gets a constraint by its name
---@param ConstraintName string
---@param bIncludesTerminated boolean
---@return ConstraintInstanceAccessor
function SkeletalMeshComponent.GetConstraintByName(ConstraintName, bIncludesTerminated) end

---Get/Set the max distance scale of clothing mesh vertices
---@return number
function SkeletalMeshComponent.GetClothMaxDistanceScale() end

---Get the current interactor for a clothing simulation, if the current simulation supports runtime interaction.
---@return ClothingSimulationInteractor
function SkeletalMeshComponent.GetClothingSimulationInteractor() end

---Returns the mass (in kg) of the given bone
---@param BoneName string
---@param bScaleMass boolean
---@return number
function SkeletalMeshComponent.GetBoneMass(BoneName, bScaleMass) end

---Get Bone Linear Velocity
---@return Vector
function SkeletalMeshComponent.GetBoneLinearVelocity() end

---Returns the animation instance that is driving the class (if available). This is typically an instance of
---the class set as AnimBlueprintGeneratedClass (generated by an animation blueprint)
---Since this instance is transient, it is not safe to be used during construction script
---@return AnimInstance
function SkeletalMeshComponent.GetAnimInstance() end

---Get Animation Mode
---@return integer
function SkeletalMeshComponent.GetAnimationMode() end

---Get Allow Rigid Body Anim Node
---@return boolean
function SkeletalMeshComponent.GetAllowRigidBodyAnimNode() end

---Get Allowed Anim Curve Evaluate
---@return boolean
function SkeletalMeshComponent.GetAllowedAnimCurveEvaluate() end

---Get Allow Cloth Actors
---@return boolean
function SkeletalMeshComponent.GetAllowClothActors() end

---Used to indicate we should force 'teleport and reset' during the next call to UpdateClothState.
---This can be used to reset it from a bad state or by a teleport where the old state is not important anymore.
---@return nil
function SkeletalMeshComponent.ForceClothNextUpdateTeleportAndReset() end

---Used to indicate we should force 'teleport' during the next call to UpdateClothState,
---This will transform positions and velocities and thus keep the simulation state, just translate it to a new pose.
---@return nil
function SkeletalMeshComponent.ForceClothNextUpdateTeleport() end

---Find Constraint Name from index
---@param ConstraintIndex integer
---@return string
function SkeletalMeshComponent.FindConstraintBoneName(ConstraintIndex) end

---Clear all Morph Target that are set to this mesh
---@return nil
function SkeletalMeshComponent.ClearMorphTargets() end

---Break a constraint off a Gore mesh.
---@param Impulse Vector
---@param HitLocation Vector
---@param InBoneName string
---@return nil
function SkeletalMeshComponent.BreakConstraint(Impulse, HitLocation, InBoneName) end

---If this component has a valid LeaderPoseComponent then this function makes cloth items on the follower component
---take the transforms of the cloth items on the leader component instead of simulating separately.
---@Note This will FORCE any cloth actor on the leader component to simulate in local space. Also
---The meshes used in the components must be identical for the cloth to bind correctly
---@return nil
function SkeletalMeshComponent.BindClothToLeaderPoseComponent() end

---Allow Anim Curve Evaluation
---@param NameOfCurve string
---@param bAllow boolean
---@return nil
function SkeletalMeshComponent.AllowAnimCurveEvaluation(NameOfCurve, bAllow) end

---Add impulse to all single rigid bodies below. Good for one time instant burst.
---@param Impulse Vector
---@param BoneName string
---@param bVelChange boolean
---@param bIncludeSelf boolean
---@return nil
function SkeletalMeshComponent.AddImpulseToAllBodiesBelow(Impulse, BoneName, bVelChange, bIncludeSelf) end

---Add a force to all rigid bodies below.
---This is like a 'thruster'. Good for adding a burst over some (non zero) time. Should be called every frame for the duration of the force.
---@param Force Vector
---@param BoneName string
---@param bAccelChange boolean
---@param bIncludeSelf boolean
---@return nil
function SkeletalMeshComponent.AddForceToAllBodiesBelow(Force, BoneName, bAccelChange, bIncludeSelf) end

---Add a collision source for the cloth on this component.
---Each cloth tick, the collision defined by the physics asset, transformed by the bones in the source
---component, will be applied to cloth.
---@param InSourceComponent SkeletalMeshComponent
---@param InSourcePhysicsAsset PhysicsAsset
---@return nil
function SkeletalMeshComponent.AddClothCollisionSource(InSourceComponent, InSourcePhysicsAsset) end

---Accumulate AddPhysicsBlendWeight to physics blendweight for all of the bones below passed in bone to be simulated
---@param AddPhysicsBlendWeight number
---@param bSkipCustomPhysicsType boolean
---@return nil
function SkeletalMeshComponent.AccumulateAllBodiesBelowPhysicsBlendWeight(AddPhysicsBlendWeight, bSkipCustomPhysicsType) end

return SkeletalMeshComponent
